...
class ParseBAM:
	...
	def stat (self):
		...
		print >>sys.stderr,"\n#=================================================="
		print >>sys.stderr, "%-30s%d" % ("Total Records:",R_total)
		print >>sys.stderr, "\n",
	...
	def bamTowig(self,outfile,chrom_sizes, skip_multi=True,strand_rule=None,WigSumFactor=None):
	...
	def mRNA_inner_distance(self,outfile,refbed,low_bound=0,up_bound=1000,step=10):
		...
		print >>RS, 'frag_sd = round(sd(fragsize))'
		print >>RS, 'frag_mean = round(mean(fragsize))'
		print >>RS, 'frag_median = round(median(fragsize))'
		print >>RS, 'write(c("Mean insert size",frag_mean), stderr())'
		print >>RS, 'write(c("Median insert size",frag_median), stderr())'
		print >>RS, 'write(c("Standard deviation",frag_sd), stderr())'
